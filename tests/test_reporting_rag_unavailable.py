...
def test_rag_unavailable_does_not_break_generation():
    result, context, incident_dir = run_agent('rag_unavailable_case', {'REPORTING_FORCE_RAG_FAILURE':'true'})
    assert result['report_status'] == 'ready_for_analyst_review'
    assert result['rag_used'] is False
    assert result['rag_status'] == 'forced_failure_for_test'
    assert (incident_dir/'technical_findings.md').exists()
